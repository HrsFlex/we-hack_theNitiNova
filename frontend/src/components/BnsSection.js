...
const BnsSection = () => {
  ...
  const filteredData = data.filter((item) =>
    [item.bnsSection, item.ipcSection, item.briefInfo]
      .join(" ")
      .toLowerCase()
        ...
        {filteredData.map((item, index) => (
          ...
          >
            <h2 className="text-lg font-semibold text-white">
              Section : <span>{item.bnsSection}</span>
            </h2>
            <p className="text-sm text-gray-300 mt-1">
              <span className="font-bold">IPC Section: </span>
              {item.ipcSection}
            </p>
            <p className="text-sm text-gray-400 mt-2">
              {item.briefInfo.length > 100
                ? `${item.briefInfo.slice(0, 100)}...`
                : item.briefInfo}
            </p>
            {item.briefInfo.length > 100 && (
              <button
          ...
          <div className="bg-white p-6 rounded-lg shadow-lg max-w-md w-full relative">
            ...
            <div className="overflow-y-auto max-h-96">
              <h2 className="text-lg font-semibold mb-4">
                {selectedItem.bnsSection}
              </h2>
              <p className="text-sm text-gray-700">
                <span className="font-bold">Title: </span>
                {selectedItem.ipcSection}
              </p>
              <p className="text-sm text-gray-600 mt-4">
                {selectedItem.briefInfo}
              </p>
            </div>
          </div>
        </div>
